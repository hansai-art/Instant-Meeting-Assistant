...
export function exportToMarkdown(
  ...
  lines.push(`## 會議資訊`);
  lines.push('');
  lines.push(`- **模式**: ${modeConfig.icon} ${modeConfig.name}`);
  ...
  if (options.includeStats) {
    ...
    if (session.suggestions.length > 0) {
      ...
      for (const [type, count] of Object.entries(byType)) {
        const icon = SUGGESTION_ICONS[type as SuggestionKind];
        const name = SUGGESTION_NAMES[type as SuggestionKind];
        lines.push(`  - ${icon} ${name}: ${count}`);
      }
    }
    lines.push('');
  }
  ...
  if (options.includeTranscription && session.transcriptionSegments.length > 0) {
    ...
    for (const segment of session.transcriptionSegments) {
      if (!segment.isFinal) continue; // 只包含最終結果
  ...
  if (options.includeSuggestions && session.suggestions.length > 0) {
    ...
    for (const suggestion of session.suggestions) {
      ...
      if (options.includeTimestamps) {
        lines.push(`*${formatTime(suggestion.timestamp)}*`);
        lines.push('');
      }
